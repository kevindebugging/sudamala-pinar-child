...
function child_enqueue() {
    ...
    wp_enqueue_script( 'bootstrap_js', 'http://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/js/bootstrap.min.js', array( 'jquery' ), '1.0', true );
		...
		wp_enqueue_script( 'splash-availability-form', get_stylesheet_directory_uri() . '/js/splash-availability-form.js', array( 'jquery' ), '1.0', true );
wp_enqueue_script( 'rawgit-vunit', 'https://cdn.rawgit.com/joaocunha/vunit/master/dist/vunit-0.2.0.min.js' );
wp_enqueue_script( 'vunit', get_stylesheet_directory_uri() . '/js/vunit.js', array( 'rawgit-vunit' ) );
...
add_shortcode('sudamala-main-slider', 'sudamala_main_slider');
...
add_shortcode('sudamala-call-to-action', 'sudamala_call_to_action');
...
function sudamala_splash_availability_form($attr)
	{
		...
		for ($i=1; $i < 7; $i++)
	    {
	    	...
	    	$action_link = "http://www.thebookingbutton.com.au/properties/sudamaladirect";
				...
				$pinar_availability_form_str = '
					<div class="booking-form-container container splash">
						<div id="main-booking-form" class="'.esc_attr($pinar_availability_form_attr['style']).' sudamala-booking">
							...
							<form id="splashform" class="booking-form clearfix" action="'. $action_link .'" method="get" autocomplete="off" target="_blank">
							<div class="booking-fields col-xs-6 col-md-4">
								<select id="propertyid" name="id" style="display: none;">
									<option value="sudamaladirect">Select Location</option>
									<option value="sudamaladirect">Bali</option>
									<option value="villasenggigidirect">Lombok</option>
								</select>
							<div class="chosen-container chosen-container-single" title="">
							<a class="chosen-single" tabindex="-1"><span>Select Location</span><div><b></b></div></a><div class="chosen-drop"><div class="chosen-search"><input type="text" autocomplete="off"></div><ul class="chosen-results"></ul></div></div>
							</div>
				...
				$pinar_availability_form_str = '
					<div class="booking-form-container container">
						<div id="main-booking-form" class="'.esc_attr($pinar_availability_form_attr['style']).' sudamala-booking">
							...
							<div class="input-daterange clearfix col-md-8">
						            <div class="booking-fields col-xs-6 col-md-6">
						                <input placeholder="'.esc_attr__('Choose check in date', 'ravis').'" class="datepicker-fields check-in" type="text" name="check_in_date" />
						                <i class="fa fa-calendar"></i>
						            </div>
...
/**
...

	{
		...
		$images = explode(', ', $pinar_main_gallery_attr['images']);
...

		{
			...
			$pinar_main_gallery_code .='
...

				{
					...
					$pinar_main_gallery_code .='<div class="gallery-container gallery-grid sudamala-main-gallery no-hover">
						...
						$pinar_main_gallery_code .='
							...
							<li class="item '.( (isset($image->post_title) && $image->post_title !=='' && strpos($image->post_title, 'col-') !== false ) ? esc_attr($image->post_title) : 'col-xs-6 col-md-3').' '.(isset($image->post_content) && $image->post_content !=='' ? esc_attr($image->post_content) : '').'">
								...
								<figure>
									...
									<img src="'./*esc_url( $image->guid )*/$images[$count].'" alt="11"/>

									<a href="'./*esc_url( $image->guid )*/$images[$count].'" class="more-details" data-title="'.esc_attr($image->post_excerpt).'"></a>
		...
		$art_space_body_code = '
			<div class="room-boxes wow fade '. esc_attr( $art_space_body['class'] ) .'">
				...
				<div class="room-details col-xs-6 col-md-5">
					...
					<p>
					<a href="'. esc_attr( $art_space_body['link'] ) .'" class="btn btn-default">
					'. esc_attr( $art_space_body['button-text'] ) .'</a>
					</p>
				</div>
			';

			if ($art_space_body['hover-text'] != '') {
				$art_space_body_code .= '
				<div class="price-container col-xs-6 col-md-7">
					<div class="price">
						<p>'. esc_attr( $art_space_body['hover-text'] ) .'</p>
					</div>
				</div>
				';
			}

			$art_space_body_code .= '</div>';
...
/**
	...
	function sudamala_page_slider($attr)
	{
		...
		$pinar_main_slider_code ='';
			...
			$pinar_main_slider_code .='
				<div id="main-slider" class="'. esc_attr( $sudamala_page_slider['class'] ) .'">';
	 ...
	 * ------------------------------------------------------------------------------------------
	 */
	function sudamala_main_slider($attr)
	{
		global $pinar_opt;
		$pinar_main_slider_code ='';
		...
		if (isset($pinar_opt["pinar-main-slider"]))
		{
			...
			if($blog_id == 1){
				$pinar_main_slider_code .='<div class="ta" style="width:305px;">'. $ta_bali . $ta_lombok .'</div>';
			}elseif($blog_id == 2){
				$pinar_main_slider_code .='<div class="ta" style="width:150px;">'. $ta_bali .'</div>';
			}elseif($blog_id == 4){
				$pinar_main_slider_code .='<div class="ta" style="width:150px;">'. $ta_lombok .'</div>';
			}
	...
	function sudamala_packages($attr)
	{
		...
		if ( $pinar_packages_query->have_posts() )
		{
			...
			{
				$pinar_packages = '
				<div id="special-packages" class="container sudamala-special-packages bold">
					<div class="heading-box">
	...
	function sudamala_call_to_action($attr)
	{
		/**
		...
		echo '
			...
			<div class="call-to-action-box">
				<div class="inner-container container">
					...
					<a href="'.esc_url($pinar_call_to_action_attr['link']).'" class="btn btn-secondary" target="_blank">'.esc_html($pinar_call_to_action_attr['button_text']).'</a>
				</div>
			</div>';
...
function sudamala_luxury_rooms($attr)
{
	...
	$pinar_luxury_rooms_attr = shortcode_atts(
		array(
			'title'      => esc_html__('Elegant Suites', 'ravis'),
			'subtitle'   => esc_html__('Suites combining unique traditional and fine modern touches, and warm, personalised service awaits you
', 'ravis'),
			'room_count' => 5
		), $attr );
	...
	if ( $pinar_luxury_rooms->have_posts() )
	{
		...
		<div id="luxury-rooms" class="sudamala-luxury-room bold">
			<div class="heading-box">
				...
				<div class="room-boxes '.($i%2 == 0 ? 'right' : '').' wow fadeInLeft">
					<div class="img-container col-xs-6 col-md-7">';
						if($room_cover != '')
	                    {
												//echo '' . $room_cover;
	                    	$pinar_luxury_rooms_str .=
												wp_get_attachment_image( $room_cover,
														'full',
														'', 'class=room-img' );
	                    }
	                    else
	                    {
	                    	$pinar_luxury_rooms_str .=
												'<img src="'. esc_url ( PINAR_IMG_PATH ).'room-placeholder.jpg" alt="'. esc_attr( esc_html__('No Image','ravis') ).'" />';
	                    }
                    $pinar_luxury_rooms_str .= '
                    </div>
					<div class="room-details col-xs-6 col-md-5">
